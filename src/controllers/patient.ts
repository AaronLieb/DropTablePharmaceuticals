...
const postPatient = (req: Request, res: Response) => {
    ...
    let { ssn, firstName, lastName, phone, streetName, streetNumber, city, zip, insurance, username } = req.body;
    mysql.db.query('INSERT INTO patient VALUES (?)', [[ssn, firstName, lastName, streetName, streetNumber, city, zip, insurance, username, phone]],
     (error, result) => {
        if (error) {
            logging.error(NAMESPACE, 'Could not perform query', error);
            res.status(400).json(error);
...
const updatePatient = (req: Request, res: Response) => {
    logging.info(NAMESPACE, `updatePatient endpoint called.`);
